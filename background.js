...
chrome.runtime.onMessage.addListener(
    function(request, sender, sendResponse) {
        ...
        if (request.type === 'showPageAction') {
            ...
            searchClassProfessor(request, sender, sendResponse);
            return true;
        }
    });

function searchClassProfessor(request, sender, sendResponse) {
    ...
    $.get('https://www.ratemyprofessors.com/search.jsp?query=' + name + "+Arizona+State+University", function(response) {
        var link = "https://www.ratemyprofessors.com" + $(response).find('ul[class="listings"]').children().first().children().first().attr("href");
        getProfessorRatings(request, sender, sendResponse, link)
    });
}

function getProfessorRatings(request, sender, sendResponse, link) {
    console.log(link)
